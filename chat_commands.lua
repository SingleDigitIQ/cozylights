...
local clearlights = {
	...
	func = function(name, param)
		...
		if not player then
			return false, "Requires active player entity."
		end
	...
	func = function(name, param)
		...
		for i in a:iterp(minp, maxp) do
			...
			if cozy_item ~= nil then
				local radius, _ = cozylights:calc_dims(node_name, cozy_item)
				local posrebuild = a:position(i)
				if
					vector.in_area(vector.subtract(posrebuild, radius), minp, maxp)
					and vector.in_area(vector.add(posrebuild, radius), minp, maxp)
				then
					cozylights:draw_node_light(posrebuild, cozy_item, cozy_item.name, vm, a, data, param2data)
				else
		...
		for i in a:iterp(minp, maxp) do
			...
			if cozy_item ~= nil then
				...
				if
					...
					and vector.in_area(vector.add(posrebuild, radius), minp, maxp)
				then
					cozylights:draw_node_light(posrebuild, cozy_item, cozy_item.name, vm, a, data, param2data, true)
				else
					local single_light_queue = cozylights.single_light_queue
					single_light_queue[#single_light_queue + 1] = {
						pos = posrebuild,
...
local spawnlight = {
	...
	func = function(name, param)
		...
		if not brightness or not radius or not strength then
			return false, "Missing arguments. Usage: /spawnlight <brightness> <radius> <strength>"
		end
	...
	func = function(name)
		local settings_formspec = {
			...
			"tooltip[0.95,1.8;3.4,0.5;Cozy Lights Wield Light step - smaller value will result in more frequent, fluid wield light update.\n"
				.. "Smaller values might be too expensive for potato. Valid values are from 0.05 to 1.00]",
			...
			"label[0.95,4.15;Global Brightness]",
			"field[3.5,3.9;0.8,0.5;global_brightness;;" .. cozylights.global_brightness .. "]",
			"tooltip[0.95,3.9;3.4,0.5;Determines how bright overall(relative to own light source brightness) the light will be.\n"
				.. "Affects placed nodes(like torches, mese lamps, etc) and wielded light, but not light brush.\n"
				.. "Valid values are from -10.0 to 10.0.]",

			"label[0.95,4.85;Global Radius]",
			"field[3.5,4.6;0.8,0.5;global_radius;;" .. cozylights.global_radius .. "]",
			"tooltip[0.95,4.6;3.4,0.5;Determines how far light of all light source nodes will reach.\n"
				.. "Affects placed nodes(like torches, mese lamps, etc) and wielded light, but not light brush.\n"
				.. "Valid values are from 1.0 to 50.0.\n"
			...
			"label[0.95,5.55;Global Strength]",
			"field[3.5,5.3;0.8,0.5;global_strength;;" .. cozylights.global_strength .. "]",
			"tooltip[0.95,5.3;3.4,0.5;Determines the gradient, how slowly the brightness fades farther from the source.\n"
				.. "Affects placed nodes(like torches, mese lamps, etc) and wielded light, but not light brush.\n"
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	if formname ~= "cozylights:settings" then
		return
	end
	if player == nil then
		return
	...
	if fields.wielded_light_radius then
		local wielded_light_radius = tonumber(fields.wielded_light_radius) > 30 and 30
			or tonumber(fields.wielded_light_radius)
	...
	if fields.wield_step then
		...
		wield_step = wield_step < 0.05 and 0.05 or wield_step
	...
	if fields.global_brightness then
		local brightness = tonumber(fields.global_brightness) > 10 and 10 or tonumber(fields.global_brightness)
		cozylights.global_brightness = brightness < -10 and -10 or brightness or 3
		minetest.settings:set("cozylights_global_brightness", cozylights.global_brightness)
	end
	if fields.global_radius then
		local radius = tonumber(fields.global_radius) > 50 and 50 or tonumber(fields.global_radius)
		cozylights.global_radius = radius < 1 and 1 or radius or 4
		minetest.settings:set("cozylights_global_radius", cozylights.global_radius)
	end
	if fields.global_strength then
		local strength = tonumber(fields.global_strength) > 1 and 1 or tonumber(fields.global_strength)
		cozylights.global_strength = strength < 0 and 0 or strength or 0.4
		minetest.settings:set("cozylights_global_strength", cozylights.global_strength)
	end
...
local daynightratio = {
	...
	func = function(name, param)
		...
		if not ratio then
			return false, "Missing argument. Usage: /daynightratio <ratio>"
		end
...
local cozyadjust = {
	...
	func = function(name, param)
		...
		local size = mf(tonumber(args[1]) or cozylights.default_size)
		local adjust_by = mf(tonumber(args[2]) or 1)
		local keep_map = mf(tonumber(args[3]) or 1)
		minetest.log("action", name .. " uses /cozyadjust " .. size .. " at position: " .. cozylights:dump(pos))
		if size > 120 then
			return false, "Radius is too big"
		end
		local minp, maxp, vm, data, param2data, a = cozylights:getVoxelManipData(pos, size)
